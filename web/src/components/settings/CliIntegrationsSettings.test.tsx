  ...
  integrations: [
    {
      ...
      installed: true,
      version: "v22.12.0",
      path: "/app/glimmer-node",
      source: "bundled",
      agentAccess: "runtime",
      detail: "Bundled with Glimmer.",
    },
    {
      id: "github_cli",
      name: "GitHub CLI",
...
describe("CliIntegrationsSettings", () => {
  ...
  it("copies a visible sign-in command and reruns checks on request", async () => {
    ...
    await waitFor(() =>
      expect(writeText).toHaveBeenCalledWith("gh auth login -h github.com -p https -w"),
    );
    expect(await screen.findByRole("button", { name: /Copy sign-in command:/i })).toHaveTextContent(
      "Copied",
    );
