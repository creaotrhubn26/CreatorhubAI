...
function rowMatchesFilter(row: TimelineRow, filter: Filter): boolean {
  ...
  if (filter === "Agent")
    return (
      e.type === "agent_state_changed" ||
      e.type === "candidate_selected" ||
      e.type === "scope_expanded" ||
      e.type === "model_request_started"
    );
  ...
  if (filter === "Verification")
    return e.type === "verification_started" || e.type === "verification_completed";
...
function describe(e: GlimmerEvent): string {
  switch (e.type) {
    ...
    case "tool_blocked":
      return `SECURITY BLOCK ${e.command}`;
    case "file_changed":
      return `EDIT ${e.path}`;
    case "verification_started":
      return `VERIFY ${e.command}`;
    case "verification_completed":
      return `VERIFY ${e.check}: ${e.status}`;
    case "agent_state_changed":
      return `STATE ${e.state}`;
    case "candidate_selected":
      return `DECISION ${e.file}`;
    ...
    case "scope_expanded":
      return e.approved
        ...
        : "SCOPE EXPANSION";
    case "repair_started":
      return `REPAIR ${e.iteration}`;
    case "parser_recovery":
      return `PEG retry (attempt ${e.attempt})`;
    case "model_request_started":
      return `MODEL ${e.role} → ${e.providerId}/${e.modelId}`;
...
function iconMeta(row: TimelineRow): { glyph: string; color: string } {
  ...
  switch (e.type) {
    case "tool_completed":
      return { glyph: "⚙", color: "var(--blue)" };
    case "tool_blocked":
      return { glyph: "⛔", color: "var(--red)" };
    case "file_changed":
      return { glyph: "✎", color: "var(--accent-strong)" };
    case "verification_started":
      return { glyph: "◎", color: "var(--amber)" };
    case "verification_completed":
      return { glyph: "◎", color: statusColor(e.status) };
    case "agent_state_changed":
      return { glyph: "▸", color: "var(--text-secondary)" };
    case "candidate_selected":
      return { glyph: "◆", color: "var(--accent-strong)" };
    ...
    case "scope_expanded":
      return { glyph: "⚠", color: e.approved ? "var(--amber)" : "var(--red)" };
    case "repair_started":
      return { glyph: "↻", color: "var(--amber)" };
    case "parser_recovery":
      return { glyph: "⟲", color: "var(--gray)" };
    case "model_request_started":
      return { glyph: "◇", color: "var(--blue)" };
    case "session_completed":
      return { glyph: "■", color: statusColor(e.status) };
...
function eventDetails(e: GlimmerEvent): Record<string, unknown> {
  ...
  switch (e.type) {
    case "tool_started":
      return { ...base, tool: e.tool, args: e.args };
    case "tool_completed":
      return { ...base, tool: e.tool, resultSummary: e.resultSummary };
    case "tool_blocked":
      return { ...base, command: e.command, reason: e.reason };
    case "file_changed":
      return { ...base, path: e.path, changeType: e.changeType };
    case "verification_started":
      return { ...base, command: e.command };
    case "verification_completed":
      return { ...base, check: e.check, status: e.status, baselineAware: e.baselineAware };
    case "agent_state_changed":
      return { ...base, state: e.state };
    case "candidate_selected":
      return { ...base, file: e.file, reasons: e.reasons };
    case "scope_expanded":
      return {
        ...base,
        expected: e.expected,
    ...
    case "repair_started":
      return { ...base, iteration: e.iteration };
    case "parser_recovery":
      return { ...base, attempt: e.attempt, payloadPath: e.payloadPath };
    case "model_request_started":
      return {
        ...base,
    ...
    default:
      return { ...base, ...(e as unknown as Record<string, unknown>) };
  }
}

function toolRowDetails(
  started: ToolStartedEvent,
  completed?: ToolCompletedEvent,
    ...
    <div className="card">
      ...
      <ul>
        {rows.map((row) => {
          ...
          const previewText =
            row.kind === "tool" && row.completed ? preview(row.completed.resultSummary) : undefined;
          const details =
            row.kind === "tool"
              ? toolRowDetails(row.started, row.completed)
              : eventDetails(row.event);

          return (
            <li key={row.id} className={`timeline-row tl-row${row.indent ? " indent" : ""}`}>
              <span className="tl-icon" style={{ ["--tl-color" as any]: color }} aria-hidden="true">
                {glyph}
              </span>
